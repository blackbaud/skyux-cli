...
const logger = require('@skyux/core-logger');
...
/**
 * Sets the latest versions of @skyux/core + @skyux-sdk/builder and to the package.json.
 */
const getLatestVersions = () => Promise.all([
  latestVersion('@skyux/core'),
  latestVersion('@skyux-sdk/builder')
]);
...
const cleanupTemplate = (skyux, builder) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (settings.template.name === 'library') {
      packageJson.peerDependencies['@skyux/core'] = `^${skyux}`;
      packageJson.devDependencies['@skyux/core'] = skyux;
      // Ensure libraries do not have a hard dependency on SKY UX.
      delete packageJson.dependencies['@skyux/core'];
    } else {
      packageJson.dependencies['@skyux/core'] = skyux;
    }

    packageJson.devDependencies['@skyux-sdk/builder'] = builder;
    ...
    logger.info(`Setting @skyux/core version ${skyux}`);
    logger.info(`Setting @skyux-sdk/builder version ${builder}`);
