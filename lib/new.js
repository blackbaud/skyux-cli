...
const checkoutBranch = () => {
  ...
  logger.info('Switching to branch feature-new-project.');
  const npmProcess = spawn('git', ['checkout', '-b', 'feature-newproject'], {
    cwd: settings.path,
    stdio: 'inherit'
  });
  ...
  return new Promise((resolve, reject) => {
    npmProcess.on('exit', (code) => {
      if (code !== 0) {
        reject('Switching to branch feature-newproject failed.');
        return;
      }
