...
const stdioFormat = logger.logLevel === 'verbose' ? 'inherit' : 'ignore';
...
/**
 * Determines whether we're using internal or ora logger.
 */
...
const checkoutBranch = () => {
  ...
  const message = getLogger('Switching to branch initial-commit.');
 ...
 */
const npmInstall = () => {
  const message = getLogger('Running npm install (can take several minutes)');
 ...
 */
const cloneTemplate = () => {
  const message = getLogger(`Cloning ${settings.template.name} SKY UX template.`);
  return new Promise((resolve, reject) => {
...
const cloneRepo = () => {
  ...
  return new Promise((resolve, reject) => {
    const message = getLogger('Cloning your repository.');
