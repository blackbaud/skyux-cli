...
const npmInstall = () => {
  ...
  npmProcess.on('exit', function () {
    ...
    logger.info('Change into that directory and run "sky-pages serve" to begin.');
  });
};
  ...
  clone(settings.url, settings.path, (err) => {
    if (err) {
      ...
      if (!isRepoEmpty(settings.path)) {
        logger.info('sky-pages new only works with empty repositories.');
      } else {
        cloneTemplate();
      }
    }
...
const promptForName = (args) => {
  ...
  const validator = (value) => {
    ...
    } else if (fs.existsSync(path.join('.', 'sky-pages-spa-' + value))) {
      throw new Error(
  ...
  const handler = (err, value) => {
    settings.spa = value;
    settings.name = 'sky-pages-spa-' + value;
