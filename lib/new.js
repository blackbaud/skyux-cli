...
const checkoutBranch = () => {
  ...
  const npmProcess = spawn('git', ['checkout', '-b', 'initia-commit'], {
    cwd: settings.path,
    stdio: 'inherit'
  });
 ...
 * @name npmInstallSingle
 */
const npmInstallSingle = (args) => {
  args = args || [];

  logger.info('Running npm install' + (args.length ? ' ' + args.join(' ') : ''));
  const npmProcess = spawn('npm', ['install'].concat(args), {
    cwd: settings.path,
    stdio: 'inherit'
  });
...
const nvmUse = () => {
  logger.info('Running nvm use');
  const nvmProcess = spawn('nvm', ['use'], {
    cwd: settings.path,
    stdio: 'inherit'
  });

  return new Promise((resolve, reject) => {
    nvmProcess.on('exit', (code) => {
      if (code !== 0) {
        reject('nvm use failed.');
        return;
      }
...
/**
 * Installs latest version of skyux and builder, then regular npm install.
 * @name npmInstall
 */
const npmInstall = () =>
  nvmUse()
    .then(() => npmInstallSingle())
    .then(() => npmInstallSingle(['@blackbaud/skyux@latest', '--save', '--save-exact']))
    .then(() => npmInstallSingle(['@blackbaud/skyux-builder@latest', '--save-dev', '--save-exact']))
    .then(() => {
